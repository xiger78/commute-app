# 出退勤管理アプリ

**プログラム名:** 出退勤管理（Commute Manager）  
**バージョン:** 1.0.0  
**パッケージID:** `com.commuteapp`

出勤日指定、出退勤時刻入力、出勤履歴確認、日本の祝日確認、設定（多言語・出勤タイプ・休憩時間・勤怠表CSV出力・メール送信）機能を提供する React Native モバイルアプリです。

---

## 開発環境および関連パッケージ

### 必要な開発環境

| 項目 | バージョン |
|------|-----------|
| Node.js | 18 以上（20.x 推奨） |
| npm | 8 以上 |
| JDK | 17（Android APK ビルド用） |
| Android SDK | API 34（Android 14） |
| Android Build Tools | 34.x |

### コアフレームワーク

| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| expo | ~51.0.28 | React Native フレームワーク・ビルドツール |
| react | 18.2.0 | UI ライブラリ |
| react-native | 0.74.5 | モバイルランタイム |
| typescript | ~5.3.3 | 型安全な開発 |

### ナビゲーション・UI

| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| @react-navigation/native | ^6.1.18 | アプリナビゲーション |
| @react-navigation/material-top-tabs | ^6.6.14 | 上部タブメニュー |
| react-native-tab-view | ^3.5.2 | タブビューコンポーネント |
| react-native-pager-view | 6.3.0 | スワイプ可能タブ |
| react-native-safe-area-context | 4.10.5 | セーフエリアレイアウト |
| react-native-screens | 3.31.1 | ネイティブスクリーンコンテナ |
| @react-native-picker/picker | 2.7.5 | 年月日ピッカー |

### データ・ストレージ

| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| @react-native-async-storage/async-storage | 1.23.1 | ローカルデータ永続化 |

### 設定機能（出力・メール）

| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| expo-file-system | ~17.0.1 | CSV ファイル作成 |
| expo-sharing | ~12.0.1 | CSV ファイル共有・保存 |
| expo-mail-composer | ~13.0.1 | ネイティブメール作成 |
| expo-document-picker | ~12.0.2 | ファイル添付選択 |

### インストール・実行

```bash
nodebrew use v20.18.0   # または Node 18 以上
npm install
npm run android:emu     # Android エミュレーター
npm start               # Expo 開発サーバー
```

### APK ビルド

```bash
npm run build:apk
# 出力: dist/出退勤管理-v1.0.0.apk
```

リポジトリにはビルド済み APK も含まれています:

```
dist/出退勤管理-v1.0.0.apk
```

---

## 対応 Android バージョン

| | |
|---|---|
| **最小バージョン** | Android 6.0（API 23、Marshmallow） |
| **ターゲットバージョン** | Android 14（API 34） |
| **コンパイル SDK** | API 34 |

本アプリは **Android 6.0 以降** で動作します。Android 14 向けに最適化されています。

---

## 機能別説明

アプリは **上部のかわいいバナー** と、バナー画像上の **6 つのリンクボタンメニュー** で構成されます。メニュー名は **通知 · 日付指定 · 出退時間 · 履歴確認 · 今年祝日 · 設定** です。デフォルト表示言語は **日本語** です（設定で中国語・韓国語・英語に変更可能）。

**他言語マニュアル:** [한국어](README_KO.md) · [中文](README_ZH.md) · [English](README.md)

---

### 1. 通知（出勤日メモ）

出勤日に入力した特記事項メモを日付別カードで表示します。

**使い方:**
- **日付指定**で出勤日に指定し、**出退時間**画面でメモを入力・保存した日のみ表示
- 形式: `YYYY/MM/DD(曜日):出勤種別(出勤時刻)` / `メモ:内容`
- 出勤時刻は出退時間の保存値を優先、なければ設定の出勤タイプ既定時刻
- 例:
```
2026/06/11(木):通常出勤(09:00)
メモ:当日混紡データ修正リリース作業あり
```
- 新しい日付が上に表示されます

![通知](docs/images/ja/screen-notifications.png)

---

### 2. 出勤日指定

月間カレンダーで出勤日と出勤タイプを選択します。

**使い方:**
- **年・月** を出勤履歴確認画面と同じドロップダウンピッカーで選択
- 下部の **出勤タイプボタン**（通常/早/遅/在宅/休暇）を選んでから日付をタップ
- 指定した日付はタイプの **設定色** で表示
- 同じ日付を素早く 2 回タップすると解除
- **日本の祝日** はカレンダーに **赤い丸** で表示
- タイトル右 **初期化** で当月の出勤日をすべて解除
- 凡例: 5 種類の出勤タイプ色 + 赤 = 休日

**出勤タイプの初期値（設定で変更可）:**
| タイプ | 出勤 | 退勤 |
|--------|------|------|
| 通常 | 08:40 | 出勤 + 8 時間 |
| 早 | 06:00 | 16:00 |
| 遅 | 11:00 | 20:00 |
| 在宅 | 08:40 | 出勤 + 8 時間 |
| 休暇 | — | — |

![出勤日指定](docs/images/ja/screen-work-date.png)

---

### 3. 出退勤時刻入力

選択した月の各日の出退勤時刻を入力します。

**使い方:**
- **年・月** を出勤履歴確認画面と同じドロップダウンピッカーで選択
- **一括適用** 欄で出勤・退勤時刻を入力し **適用する** と **保存** を横並びでタップ
- 日別にコンパクトな **HH 時 MM 分** 入力 UI で個別修正
- 各日の **メモ** に特記事項を入力可能
- タイトル右 **再設定** で当月の全時刻・メモを **00:00** / 空にリセット
- **保存** 後、保存欄の下にプレビューリストを表示

**色の凡例（一括適用の下）:**
- **早出勤 · 通常出勤 · 遅出勤 · 在宅勤務** — 設定の出勤タイプ色と同じ

**保存プレビュー（出勤履歴確認と同じ形式）:**
- **先頭行:** `[稼働時間:合計時間]` — 日別稼働時間の合計（小数 1 桁）
- 各行: `YYYY/MM/DD(曜日) HH:MM-HH:MM (稼働時間)`、**中央揃え**
- メモがある場合 `[メモ:内容]` を追加
- 稼働時間 = 退勤 − 出勤 − **朝・昼・夕の休憩**（設定参照）、括弧内に小数（`9.0`, `9.5`）
- **休暇** 日は合計から除外

**日付表示とカード色:**
- 各行は `YYYY/MM/DD(曜日):区分` 形式
- カード色は **設定の出勤タイプ色**（通常/早/遅/在宅/休暇）
- **平日・未指定** → `:在宅`
- **土日・祝日・未指定** → `:祝日`（灰色カード）
- **土日・祝日・指定あり** → 初期 `:出勤`; 日付ラベル（▼）で **出勤/在宅** 切替
- **土日・祝日出勤** の初期時刻 **08:40–17:40**（日別編集可）

**一括適用ルール:**
- **通常・在宅** の平日のみ（**早・遅・休暇は手動のみ**）
- **土日・日本の祝日を除く**（固定祝日、ハッピーマンデー、春分・秋分、振替休日、国民の休日）
- 画面に `土日・日本の祝日を除く · 適用対象 N日` と表示

![出退勤時刻入力](docs/images/ja/screen-commute-time.png)

---

### 4. 出勤履歴確認

月間の出勤履歴を確認します。

**使い方:**
- ドロップダウンで年・月を選択
- 選択した月の一覧が自動表示されます
- **先頭行:** `[稼働時間:合計時間]` — 当月の日別稼働時間の合計
- 各行: `YYYY/MM/DD(曜日) HH:MM-HH:MM (稼働時間)` 形式、**中央揃え**
- 括弧内の稼働時間は保存プレビューと同じ（昼食・夕食休憩除外、`9.0` / `9.5` 形式）
- 例:
```
[稼働時間:160.0]
2026/06/03(水) 09:00-18:00 (8.0)
2026/06/04(木) 09:00-18:00 (8.0)
```
- カード色・区分表示は出退勤時刻画面と同じ（出勤タイプ別の色）

![出勤履歴確認](docs/images/ja/screen-attendance-history.png)

---

### 5. 今年祝日

日本の祝日を年・月単位で確認します。

![今年祝日](docs/images/ja/screen-year-holidays.png)

---

### 6. 設定

表示言語、出勤タイプ、休憩時間、勤怠表出力(CSV)、メール送信の設定を行います。

#### 6-1. 表示言語
**日本語 · 中国語 · 韓国語 · 英語** の順で選択。全画面が即座に切り替わります。

#### 6-2. 出勤タイプ・色・時刻
- **通常 / 早 / 遅 / 在宅** — 表示色と出勤時刻（退勤は自動計算、早・遅は固定退勤時刻を維持）
- **休暇** — 表示色のみ（稼働時間の合計から除外）
- カード下部の **保存** ボタン（全幅）で一括保存

#### 6-3. 休憩時間設定
- カード: **休憩時間設定(朝・昼・夕)**（カテゴリ: 休憩時間設定）
- **朝休憩 (稼働時間除外)** — 初期値 **1 時間**; 出勤が **08:00 より前** のときのみ差し引き
- **昼休み (稼働時間除外)** — 初期値 **1 時間**; **休暇** 日は除外
- **夕食休み (稼働時間除外)** — 初期値 **0 時間**
- 各項目の **時・分** を独立入力（HH 時 MM 分）、下部 **保存** で 3 つまとめて保存
- 保存した休憩時間は履歴確認・保存プレビュー・CSV 出力の稼働時間から除外

#### 6-4. 勤怠表出力(CSV)
- 出力する月を選択
- **出力** ボタン（全幅）で CSV 生成・共有（§6-3 休憩・§6-2 出勤タイプを反映）

**CSV 出力形式例:**
```
2026年 06月 出勤履歴
01日: 出勤時刻:09:00、退勤時刻:18:00、稼働時間:08時間00分
...
[総勤務時間:160時間00分]
```

#### 6-5. メール送信
- 宛先・件名・本文を入力
- ファイルを添付（出力した CSV も自動添付可能）
- **メール送信** ボタン（全幅）で端末のメールアプリが起動します

![設定](docs/images/ja/screen-settings.png)

---

## 機能変更内容

| 項目 | 内容 |
|------|------|
| メニュー構成 | **通知 · 日付指定 · 出退時間 · 履歴確認 · 今年祝日 · 設定**（6タブ） |
| プロジェクト名 | `googleCalenderApp` → **CommuteApp**（`commute-app`, `com.commuteapp`） |
| 日付指定 | **通常/早/遅/在宅/休暇** ボタンで出勤日指定、タイプ別色表示 |
| 出勤タイプ | 通常 08:40 / 早 06:00–16:00 / 遅 11:00–20:00 / 在宅 08:40 / 休暇 |
| 一括適用 | **通常・在宅** の平日のみ; 早・遅・休暇は手動; **適用する·保存** 横並び |
| 土日祝日出勤 | カレンダー指定時の初期 **08:40–17:40** |
| 色凡例 | 出退時間画面に早·通常·遅·在宅の凡例 |
| 朝休憩 | 出勤 **08:00 前** のみ差し引き（初期 1 時間） |
| 休憩時間 | 朝・昼・夕; 昼は休暇除外、夕は常に差し引き |
| 稼働時間 | **`[稼働時間:合計]`** と日別 `(9.0)` 形式 |
| 今年祝日 | 年・月で日本祝日一覧・カレンダー |
| 表示言語 | **日本語 · 中国語 · 韓国語 · 英語**（初期は日本語） |
| 画面キャプチャ | ja/ko/en/zh 各 6 画面を更新 |
| Google登録タブ | 削除 |
| APK | `dist/出退勤管理-v1.0.0.apk` |

---

## プロジェクト構成

```
CommuteApp/
├── App.tsx                    # メインアプリ・タブナビゲーション
├── src/
│   ├── screens/               # 機能画面
│   ├── components/            # バナー、カレンダー等の共通 UI
│   ├── context/               # データ・言語コンテキスト
│   ├── i18n/                  # 翻訳（ja/zh/ko/en）
│   └── utils/                 # 日付・ストレージ・CSV・祝日・出退区分ユーティリティ
├── docs/images/
│   ├── ja/                    # 日本語画面キャプチャ
│   ├── zh/                    # 中国語画面キャプチャ
│   ├── ko/                    # 韓国語画面キャプチャ
│   └── en/                    # 英語画面キャプチャ
├── assets/                    # アプリアイコン・バナー・スプラッシュ
├── android/                   # Android ネイティブプロジェクト
└── dist/                      # ビルド済み APK 出力
```

---

## ライセンス

プライベートプロジェクト
